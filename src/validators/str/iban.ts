const smellsLikeIban = (str: string) =>
...
export function IBAN(value: string) {
  ...
  const numbericed = ((m[3] + m[1] + m[2]).replace as any)(/[A-Z]/g, (ch: string) => {
  ...
  const mod97 = numbericed.match(/\d{1,7}/g).reduce((total: number, curr: number) => {
